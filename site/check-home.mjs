...
for (const file of ["home.css", "style.css"]) {
  const css = readFileSync(resolve(site, file), "utf8");
  for (const [, target] of css.matchAll(/url\(["']?([^)"']+)["']?\)/g)) {
    if (/^(?:[a-z]+:|\/\/|#)/i.test(target)) continue;
    assert(existsSync(resolve(site, target)), "Missing CSS asset: " + target);
  }
}
...
console.log("Homepage checks passed: " + checked + " local references, unique IDs, CSS assets and data-backed cards.");
